...
function createEmptyRooms(dormId, roomCount) {
  ...
  const roomSql = 'INSERT INTO rooms (dorm_id, room_number, status, price, description, floor) VALUES ?';
...
function updateRoomCount(dormId, newRoomCount, currentRoomCount) {
  ...
  console.log(`🏠 Room count update: ${currentCount} -> ${newCount} for dorm ${dormId}`);
  ...
  if (newCount > currentCount) {
    ...
    pool.query(deleteSql, [dormId, dormId, roomsToDelete], (err, result) => {
      if (err) {
        ...
        console.log(`✅ Deleted ${deletedCount} empty rooms for dorm ${dormId}`);
        
        if (deletedCount < roomsToDelete) {
          console.log(`⚠️ Could only delete ${deletedCount}/${roomsToDelete} rooms (some may be occupied)`);
        }
      }
    });
  }
  ...
  try {
    ...
    const chat = model.startChat({
      history: [
        ...
        { role: "model", parts: [{ text: "เข้าใจแล้วค่ะ! 🏠 น้องดอร์มพร้อมช่วยเหลือทุกคนที่กำลังมองหาหอพักในมหาสารคามแล้วค่ะ จะตอบคำถามอย่างเป็นธรรมชาติ เป็นกันเอง แต่ยังคงให้ข้อมูลที่ถูกต้องและเป็นประโยชน์ บอกมาได้เลยนะคะว่ามีอะไรให้ช่วย! 😊" }] },
      ],
    });
...
app.post('/distance-between-dorms', async (req, res) => {
  try {
    ...
    // ตรวจสอบว่ามีพิกัด
    if (!dorm1.latitude || !dorm1.longitude || !dorm2.latitude || !dorm2.longitude) {
      return res.status(400).json({ 
...
async function answerComparisonQuery(message, dorms) {
  ...
  for (const dorm of dorms) {
    ...
    const dormNameClean = dormNameLower.replace(/\s+/g, '').replace(/หอพัก|หอ/g, '');
  ...
  if (foundDorms.length === 2) {
    console.log('🔄 Comparison query detected:', foundDorms[0].name, 'vs', foundDorms[1].name);
    ...
    const prices1 = [];
    if (dorm1.price_monthly && Number(dorm1.price_monthly) > 0) prices1.push(`รายเดือน ฿${Number(dorm1.price_monthly).toLocaleString()}`);
    if (dorm1.price_daily && Number(dorm1.price_daily) > 0) prices1.push(`รายวัน ฿${Number(dorm1.price_daily).toLocaleString()}`);
    if (dorm1.price_term && Number(dorm1.price_term) > 0) prices1.push(`รายเทอม ฿${Number(dorm1.price_term).toLocaleString()}`);
    if (prices1.length > 0) response += `💰 ราคา: ${prices1.join(' | ')}\n`;
    if (dorm1.facilities) response += `🌟 สิ่งอำนวยความสะดวก: ${dorm1.facilities.substring(0, 100)}${dorm1.facilities.length > 100 ? '...' : ''}\n`;
    if (dorm1.near_places) response += `📍 ที่ตั้ง: ${dorm1.near_places.substring(0, 80)}${dorm1.near_places.length > 80 ? '...' : ''}\n`;
    response += `\n`;
    
    // หอพักที่ 2
    response += `**2. 🏠 ${dorm2.name}**\n`;
    const prices2 = [];
    if (dorm2.price_monthly && Number(dorm2.price_monthly) > 0) prices2.push(`รายเดือน ฿${Number(dorm2.price_monthly).toLocaleString()}`);
    if (dorm2.price_daily && Number(dorm2.price_daily) > 0) prices2.push(`รายวัน ฿${Number(dorm2.price_daily).toLocaleString()}`);
    if (dorm2.price_term && Number(dorm2.price_term) > 0) prices2.push(`รายเทอม ฿${Number(dorm2.price_term).toLocaleString()}`);
    if (prices2.length > 0) response += `💰 ราคา: ${prices2.join(' | ')}\n`;
    if (dorm2.facilities) response += `🌟 สิ่งอำนวยความสะดวก: ${dorm2.facilities.substring(0, 100)}${dorm2.facilities.length > 100 ? '...' : ''}\n`;
    if (dorm2.near_places) response += `📍 ที่ตั้ง: ${dorm2.near_places.substring(0, 80)}${dorm2.near_places.length > 80 ? '...' : ''}\n`;
    response += `\n`;
    ...
    // เปรียบเทียบราคา
    const price1 = Number(dorm1.price_monthly) || Number(dorm1.price_daily) || Number(dorm1.price_term) || 0;
    const price2 = Number(dorm2.price_monthly) || Number(dorm2.price_daily) || Number(dorm2.price_term) || 0;
    
    if (price1 > 0 && price2 > 0) {
      if (price1 < price2) {
        response += `💰 **${dorm1.name}** ราคาถูกกว่า (ต่างกัน ฿${(price2 - price1).toLocaleString()})\n`;
      } else if (price2 < price1) {
        response += `💰 **${dorm2.name}** ราคาถูกกว่า (ต่างกัน ฿${(price1 - price2).toLocaleString()})\n`;
      } else {
    ...
    const facilities1Count = dorm1.facilities ? dorm1.facilities.split(',').length : 0;
    const facilities2Count = dorm2.facilities ? dorm2.facilities.split(',').length : 0;
    ...
    if (parts.length >= 2) {
      ...
      for (const part of parts) {
        ...
        const matched = dorms.find(d => {
          const cleanDormName = d.name.toLowerCase().replace(/หอพัก|หอ/g, '').replace(/\s+/g, '');
  ...
  if (foundDorms.length >= 2) {
    try {
      ...
      const distanceResult = await calculateDistanceBetweenDorms(
        {
          name: coord1.name,
          coordinates: [{ latitude: coord1.latitude, longitude: coord1.longitude }]
        },
        {
          name: coord2.name,
          coordinates: [{ latitude: coord2.latitude, longitude: coord2.longitude }]
        }
      );
...
ตอบอย่างเป็นธรรมชาติ ยืดหยุ่น และเป็นมิตร!`;
  ...
  try {
    ...
    const chat = model.startChat({
      history: [
        { role: "user", parts: [{ text: systemPrompt }] },
        { role: "model", parts: [{ text: "เข้าใจแล้วค่ะ! น้องพร้อมช่วยหาหอพักที่ใช่สำหรับคุณ จะตอบแบบเป็นกันเองและให้ข้อมูลที่ถูกต้องค่ะ! 😊" }] },
      ],
    });
    ...
    console.log('✅ Gemini Response (Filtered):', aiResponse.substring(0, 100) + '...');
...
app.post('/chatbot', async (req, res) => {
  try {
    ...
    console.log('✅ Gemini AI Response:', aiResponse?.substring(0, 200) + '...');
