...
// ฟังก์ชันตรวจสอบและกรองคำตอบของ AI
function validateAIResponse(aiResponse, dormContext) {
  try {
    ...
    // ถ้า AI ไม่ตอบตามข้อมูลหรือมีข้อมูลที่ไม่ถูกต้อง
    if (!aiResponse || aiResponse.trim().length === 0) {
      return "ขออภัยค่ะ ไม่สามารถประมวลผลคำถามได้ กรุณาลองใหม่อีกครั้ง";
    }
    
    // ตรวจสอบว่า AI ตอบเรื่องหอพัก ราคา สถานที่ หรือสิ่งอำนวยความสะดวก
    const isRelevantTopic = 
      validDormNames.some(name => aiResponse.includes(name)) ||
      aiResponse.includes('หอพัก') ||
      aiResponse.includes('ราคา') ||
      aiResponse.includes('บาท') ||
      aiResponse.includes('ใกล้') ||
      aiResponse.includes('สิ่งอำนวยความสะดวก') ||
      aiResponse.includes('ไม่มีข้อมูล') ||
      aiResponse.includes('ขออภัย') ||
      aiResponse.includes('แนะนำ') ||
      aiResponse.includes('เสนอ') ||
      dormContext.length > 0; // ถ้ามี context แสดงว่าเป็นคำถามเกี่ยวกับหอพัก
    
    // อนุญาตให้ผ่านทุกคำตอบถ้ามี context หอพัก
    if (dormContext.length > 0) {
      return aiResponse;
    }
    
    if (!isRelevantTopic) {
      return "ขออภัยค่ะ ฉันสามารถช่วยเรื่องหอพัก ราคา สถานที่ใกล้เคียง และสิ่งอำนวยความสะดวกเท่านั้น 🏠";
    }
...
app.post('/chatbot', async (req, res) => {
  try {
    ...
    // ตอบแบบ simple rule-based ถ้าเป็นคำทักทาย
    if (/สวัสดี|hello|hi|หวัดดี/.test(message.toLowerCase())) {
      return res.json({
        message: 'สวัสดีค่ะ 😊 มีเรื่องหอพักอะไรให้ช่วยไหมคะ?',
